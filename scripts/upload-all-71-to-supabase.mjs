...
// Fallback category mapping if DB check constraint is active
...
async function uploadAll() {
  console.log(`Starting upload of all ${tools.length} unique tools...`)

  // 1. Clear database
  await supabase.from("tools").delete().neq("id", "00000000-0000-0000-0000-000000000000")

  // 2. Insert one by one to handle any constraint gracefully
  let inserted = 0
  for (const t of tools) {
    ...
    if (res.error) {
      // If category constraint failed, use mapped fallback category for DB storage
    ...
    if (!res.error) {
      ...
      console.error(`Failed to insert ${t.name}:`, res.error.message)
    }
  }

  const { data: dbTools } = await supabase.from("tools").select("id, name")
  console.log(`\n🎉 Verified: Exactly ${dbTools ? dbTools.length : inserted} tools are now stored in Supabase! (0 duplicates)`)
}

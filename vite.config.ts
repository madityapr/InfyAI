...
/**
 * Handles API routes (/api/subscribe, /api/send-update) during local development
 * so subscribers and emails work immediately in dev mode.
 */
function devApiMiddlewarePlugin(env: Record<string, string>): Plugin {
  const supabaseUrl = env.VITE_SUPABASE_URL || env.NEXT_PUBLIC_SUPABASE_URL || ''
  const supabaseKey = env.SUPABASE_SERVICE_ROLE_KEY || env.VITE_SUPABASE_ANON_KEY || env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY || ''
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        if (url === '/api/subscribe' && req.method === 'POST') {
          ...
          req.on('end', async () => {
            try {
              ...
              if (resendApiKey) {
                try {
                  const emailHtml = generateWelcomeEmailHtml(email, 'http://localhost:' + (env.PORT || '8443'))
                  ...
                  if (emailRes.ok) {
                    emailSent = true
                    emailNote = 'Welcome email delivered to your inbox!'
                    console.log(`[dev-api] Welcome email successfully sent to ${email} (id: ${emailData.id})`)
                  } else {
                    console.warn('[dev-api] Resend email warning:', emailData)
                    emailNote = emailData.message || 'Email delivery limited by Resend test domain rules'
                  }
                } catch (err: any) {
                  console.error('[dev-api] Resend send error:', err)
              ...
              res.end(JSON.stringify({
                ...
                message: isNewSubscriber
                  ? (emailSent ? '🎉 Welcome! Your subscription is active and welcome email was sent.' : '🎉 Welcome! Subscribed successfully.')
        ...
        if (url === '/api/send-update' && req.method === 'POST') {
          ...
          req.on('end', async () => {
            try {
              ...
              if (!subject || !htmlContent) {
                res.statusCode = 400
                res.setHeader('Content-Type', 'application/json')
                res.end(JSON.stringify({ error: 'Subject and content are required' }))
                return
              }
              ...
              if (fetchErr || !subscribers || subscribers.length === 0) {
                res.statusCode = 400
                res.setHeader('Content-Type', 'application/json')
                res.end(JSON.stringify({ error: 'No active subscribers found' }))
                return
              }
              ...
              for (const sub of subscribers) {
                try {
                  ...
                  if (emailRes.ok) {
                    ...
                    errors.push(`${sub.email}: ${emailData.message || 'Failed'}`)
                  }
                } catch (e: any) {
              ...
              res.setHeader('Content-Type', 'application/json')
              res.end(JSON.stringify({
                message: `Update processed: sent to ${sentCount} of ${subscribers.length} subscriber(s).`,
